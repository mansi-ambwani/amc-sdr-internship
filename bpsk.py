...
def proc_pipe(raw_signal, base_pat, head_bits, head_syms, mix_syms, rx_filter, sampling_f, sps, num_span, runs):
    ...
    def phase_track(sig_raw, sig_filtered, base_offset, h_val):
        ...
        # FIX 2: Optimized Costas Loop Bandwidth parameters to track phase drift over long streams (Reduces BER)
        bw_w = 0.05  # Tightened wide acquisition loop
        bw_n = 0.008 # Tightened narrow tracking loop to maintain lock on massive data sizes
...
def start_app():
    ...
    # Target large stream configuration
    info_bits = np.random.randint(0, 2, 150000)
    fs_rate = 20e6
    carrier_f = 1.5e9
    gain_tx = 60
    gain_rx = 30
    ...
    rx_duration = 0.08  # Increased slightly to fully clear data capture profiles
    ...
    # # FIX 1: Strict Matrix Truncation / Padding to create unified shapes for CNN Training
    # TARGET_WINDOWS = 250  # 250 windows * 1024 window_size = 256,000 exact IQ matrix points
    # WINDOW_SIZE = 1024
    # REQUIRED_LENGTH = TARGET_WINDOWS * WINDOW_SIZE
    
    # current_length = len(ml_complex_data)
    # if current_length >= REQUIRED_LENGTH:
    #     # Cut off any extra samples added by RRC delay filters or OS async capture intervals
    #     ml_complex_data = ml_complex_data[:REQUIRED_LENGTH]
    # else:
    #     # Zero-pad if transmission ends short due to sampling deviations
    ...
    #   # Transpose so rows become individual sequential samples: [I, Q]
    # sequential_samples = ml_ready_matrix.T 

    # # Check a stable window further down the timeline (e.g., sample index 100 to 105)
    # print("Actual IQ pairs (Samples 100 to 105):")
    # print(sequential_samples[100:105])

    
    # # Save perfectly formatted array out to disk
    # file_path = os.path.join("rf_dataset", "ml_bpsk_samples.npy")
    ...
    REQUIRED_LENGTH = int(150000/BITS_PER_SYM) 
    current_length = len(ml_complex_data)
    ...
    file_path = os.path.join("data_equal", "ml_bpsk_samples.npy")
    np.save(file_path, ml_ready_matrix)
