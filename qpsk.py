...
# Configuration Settings for QPSK
M = 4 
...
def proc_pipe(raw_signal, base_pat, head_bits, head_syms, mix_syms, rx_filter, sampling_f, sps, num_span, runs):
    ...
    #     # Optimized tracking gains optimized specifically for QPSK constellation tracking
...
def start_app():
    ...
    # FIXED BALANCING FORMULA: 250 target windows * 1024 size * 2 bits/sym = 512,000 bits
    info_bits = np.random.randint(0, 2, 300000)
    ...
    # file_path = os.path.join("rf_dataset", "ml_qpsk_samples.npy")
    ...
    file_path = os.path.join("data_equal", "ml_qpsk_samples.npy")
    np.save(file_path, ml_ready_matrix)
