...
def proc_pipe(raw_signal, base_pat, head_bits, head_syms, mix_syms, rx_filter, sampling_f, sps, num_span, runs):
    ...
    #             if idx < len(head_syms):
    #                 true_sym = head_syms[idx]
    #                 # Normalized Preamble phase discriminator
    #                 err_metric = np.imag(rot_sym / (true_sym if np.abs(true_sym) > 0 else 1))
    #             else:
    #                 # QAM Phase-only error discriminator (amplitude normalized)
    #                 # Prevents outer QAM constellations from dominating loop metrics
    #                 err_metric = np.real(rot_sym) * np.imag(sliced_sym) - np.imag(rot_sym) * np.real(sliced_sym)
    #                 # Normalize by distance magnitude to ensure steady gain states
    #                 mag_sq = np.abs(rot_sym) * np.abs(sliced_sym)
    #                 if mag_sq > 1e-6:
    #                     err_metric /= mag_sq
    ...
    #     # Robust Phase Ambiguity Resolution using exact preamble phase errors
    #     best_rot = 0
    #     min_errors = len(head_bits) + 1
    #     for k_rot in range(4):
    #         test_bits = np.zeros(len(head_bits), dtype=int)
    #         rot_factor = np.exp(-1j * np.pi * k_rot / 2)
    #         for h_idx in range(len(head_syms)):
    #             rotated = out_syms[h_idx] * rot_factor
    #             _, p_idx = slice_universal_qam(rotated)
    #             test_bits[h_idx*BITS_PER_SYM : (h_idx+1)*BITS_PER_SYM] = universal_sym_to_bits(p_idx)
    ...
    # FIX: Correctly maps output matrix variables without wiping loop data
...
def start_app():
    ...
    info_bits = np.random.randint(0, 2, 1200000)
    fs_rate = 20e6  
    carrier_f = 1.5e9
    gain_tx = 60
    gain_rx = 30
    ...
    # -------------------------------------------------------------------------
    # FIXED LENGTH ENFORCEMENT STAGE (Enforces exactly 256,000 output samples)
    # -------------------------------------------------------------------------
    ...
    #     # Clip down precisely if it overshoots boundary
    #     processed_complex = ml_complex_data[:target_len]
    # else:
    #     # Pad with zeros to remain unified with the PSK training files
    ...
    # file_path = os.path.join("rf_dataset", "ml_16qam_samples.npy")
 ...
 # -------------------------------------------------------------------------
    ...
    file_path = os.path.join("data_qam", "ml_16qam_samples.npy")
    np.save(file_path, ml_ready_matrix)
    print(f">>> ML Feature Capture Saved! Shape: {ml_ready_matrix.shape} to '{file_path}'")
    # -------------------------------------------------------------------------
